/**
 * TrailersList — Otoqa Web chassis for the fleet trailers screen.
 *
 * Same shape as `TrucksList`. Differences vs trucks:
 *   - Vehicle column is replaced by Type (bodyType) and Length (size) —
 *     trailers don't have a make/model that matters as much as the body type
 *   - No telematics filter (trailers aren't on the Samsara mapping today)
 *   - Click → routes to /fleet/trailers/[id]
 */
...
export interface TrailerRow {
  ...
  bodyType?: string;
  /** "53ft" | "48ft" | "40ft" — display as length */
  size?: string;
...
function needsAttentionTrailer(t: TrailerRow): boolean {
...
interface SystemView {
  id: string;
  label: string;
  predicate: (t: TrailerRow) => boolean;
  tone?: 'neutral' | 'accent' | 'warn' | 'danger';
}

const SYSTEM_VIEWS: SystemView[] = [
  { id: 'all',         label: 'All Trailers',     predicate: (t) => !t.isDeleted },
  { id: 'active',      label: 'Active',           predicate: (t) => !t.isDeleted && t.status === 'Active' },
  { id: 'attention',   label: 'Needs Attention',  predicate: needsAttentionTrailer, tone: 'warn' },
...
const COLUMNS: TableColumn<TrailerRow>[] = [
  {
    key: 'unit',
    label: 'Unit',
    width: '1.2fr',
  ...
  },
  {
    ...
    sortable: false,
    render: (t) => {
      const ymm = [t.year, t.make].filter(Boolean).join(' ');
      const headline = [t.size, t.bodyType].filter(Boolean).join(' ');
      return (
        <span className="flex flex-col leading-tight min-w-0">
          <span className="text-[13px] text-foreground truncate">{headline || '—'}</span>
          {ymm && (
            <span className="text-[11.5px] text-[var(--text-tertiary)] truncate">{ymm}</span>
          )}
        </span>
      );
    },
  ...
  },
  {
    key: 'registration',
    label: 'Registration',
    // Fixed so reg/insurance/status stay clustered on the right edge instead
    // of stretching to absorb leftover flex space (Unit + Type take that).
    width: '200px',
    render: (t) => (
...
export function TrailersList({
  ...
  const counts = React.useMemo(() => {
    ...
    for (const v of SYSTEM_VIEWS) out[v.id] = trailers.filter(v.predicate).length;
    return out;
  }, [trailers]);
  ...
  const filtered = React.useMemo(() => {
    let rows = systemView ? trailers.filter(systemView.predicate) : trailers.filter((t) => !t.isDeleted);
    for (const chip of filters) {
      ...
      rows = rows.filter((t) => {
        switch (chip.propId) {
          ...
          case 'size':         return t.size ? wanted.has(t.size) : false;
          case 'reg-status': {
    ...
    if (search.trim()) {
      ...
      rows = rows.filter(
        (t) =>
          ...
          (t.bodyType ?? '').toLowerCase().includes(q) ||
    ...
    return rows;
  }, [trailers, systemView, filters, search, sortKey, sortDir]);

  const properties: FilterProperty[] = React.useMemo(() => {
    const statusOptions = uniqueValues(trailers, (t) => t.status).map((v) => ({ value: v, label: v }));
    const bodyOptions = uniqueValues(trailers, (t) => t.bodyType).map((v) => ({ value: v, label: v }));
    const sizeOptions = uniqueValues(trailers, (t) => t.size).map((v) => ({ value: v, label: v }));
    return [
      ...
      { id: 'size',      label: 'Length',     kind: 'enum', icon: 'package',      options: sizeOptions },
      {
        id: 'reg-status',
        label: 'Registration',
        kind: 'enum',
    ...
    <div className="flex-1 flex flex-col min-h-0 relative">
      <PageHeader
        ...
        actions={
          <>
            ...
            <WBtn variant="primary" size="sm" leading="plus" onClick={onCreate}>Create Trailer</WBtn>
          </>
        }
      />
      <SavedViews views={tabs} activeId={viewId} onChange={setViewId} />
      <TableToolbar
        searchPlaceholder="Search unit, VIN, plate, type…"
        searchValue={search}
      ...
      {loading && trailers.length === 0 ? (
        <div className="flex-1 flex items-center justify-center">
          <p className="text-[12.5px] text-[var(--text-tertiary)]">Loading trailers…</p>
        </div>
      ) : filtered.length === 0 ? (
        <EmptyState
          hasData={trailers.length > 0}
          onCreate={onCreate}
          onClearFilters={() => {
            setSearch('');
            setFilters([]);
        ...
        <Table<TrailerRow>
          ...
          sortDir={sortDir}
          onSort={onSort}
          onRowClick={(r) => router.push(`/fleet/trailers/${r._id}`)}
          getRowId={(r) => r._id}
        />
      )}
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center px-6">
      <span className="h-10 w-10 rounded-full bg-[var(--bg-surface-2)] inline-flex items-center justify-center text-[var(--text-tertiary)]">
        <WIcon name="package" size={18} />
      </span>
      {hasData ? (
        <>
          <p className="m-0 text-[14px] text-foreground font-medium">No trailers match these filters</p>
        ...
        <>
          <p className="m-0 text-[14px] text-foreground font-medium">No trailers yet</p>
          <p className="m-0 text-[12.5px] text-[var(--text-tertiary)] max-w-xs">
            Add your first trailer to start tracking inspections, registration, and assignments.
          </p>
          <WBtn variant="primary" size="sm" leading="plus" onClick={onCreate}>Create Trailer</WBtn>
        </>
      )}
...
function sortValueOf(t: TrailerRow, key: string): string | number {
  switch (key) {
    case 'unit':         return t.unitId.toLowerCase();
    case 'type':         return `${t.size ?? ''} ${t.bodyType ?? ''}`.toLowerCase();
