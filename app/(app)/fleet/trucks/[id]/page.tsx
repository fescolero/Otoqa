      ...
      <DSActivity
        items={[
          ...(truck.isDeleted && truck.deletedAt
            ? [{ icon: 'trash' as const, text: `Deactivated · ${fmtDate(new Date(truck.deletedAt).toISOString().slice(0, 10))}`, when: '' }]
            : []),
          ...(truck.updatedAt !== truck.createdAt
            ? [{ icon: 'edit-pen' as const, text: `Last updated · ${fmtDate(new Date(truck.updatedAt).toISOString().slice(0, 10))}`, when: '' }]
            : []),
          { icon: 'truck' as const, text: `Truck created · ${fmtDate(new Date(truck.createdAt).toISOString().slice(0, 10))}`, when: '' },
        ]}
      />
    </DSCard>
  );
