...
export function InvoiceFilterBar({
  ...
  useEffect(() => {
    const saved = localStorage.getItem('invoice-saved-views');
    if (saved) {
  ...
  const saveSavedViews = (views: SavedView[]) => {
    localStorage.setItem('invoice-saved-views', JSON.stringify(views));
    setSavedViews(views);
  };

  const hasActiveFilters = filters.search || filters.hcr || filters.trip || filters.loadType || filters.dateRange;
  ...
  return (
    ...
    >
      ...
      <Select
        ...
        onValueChange={(value) => onFiltersChange({ ...filters, loadType: value === 'all' ? undefined : value as any })}
      >
