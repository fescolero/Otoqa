...
/**
 * Pay plans — settlement cadence manager.
 *
 * Visual reference: Otoqa Web design — settings-screen.jsx > PayPlansModal,
 * implemented as the /org-settings/pay-plans page: a plan rail on the left
 * (active plans, archived group, "New pay plan"), an editor on the right
 * (name/description, frequency with per-frequency anchor controls, pay-lag
 * stepper, live "Next 3 pay periods" preview, and an Advanced disclosure for
 * the engine fields: cutoff, timezone, payable trigger, currency, amendment
 * policy, carryover toggles), and a guarded footer (archive/restore/default).
 *
...
export default function PayPlansPage() {
  ...
  const cancel = () => {
    if (selId === 'new') {
      const first = active[0] ?? archived[0];
      setSelId(first ? first._id : null);
      if (!first) setDraft(null);
    } else if (sel) {
      setDraft(draftFromPlan(sel));
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-auto bg-[var(--bg-canvas)]">
      <SettingsHeader
        ...
        title="Pay plans"
        subtitle="How often settlements run. Assign a plan to any driver."
      />
      ...
      <div className="flex-1 p-6" style={{ minWidth: 0 }}>
        ...
        >
          <div style={{ display: 'grid', gridTemplateColumns: '218px minmax(0,1fr)', minHeight: 480 }}>
            {/* ── plan rail ─────────────────────────────────────────────── */}
            <div
          ...
          >
            ...
            <div className="flex items-center gap-2 shrink-0">
              <WBtn size="sm" onClick={cancel} disabled={!draft}>Cancel</WBtn>
