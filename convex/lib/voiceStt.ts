...
- assign follow-ups MERGE with the assign being built: when the conversation shows an assign that was missing a piece, failed, or is being adjusted, keep its established trips/hcr/driver/dates and change only what the new utterance supplies. "assign it to Jorge Romero" after "Which driver…?" → the full earlier assign with driverQuery Jorge Romero. "the date is august 1st" after a no-loads-found reply → the same trips/hcr/driver with dates ["…-08-01"]. "can we also assign 1 and 2 for saturday" after assigning to Andres → same driver and hcr, new trips/dates.
- NEVER output placeholder values like "<UNKNOWN>" or "N/A" for any field — omit the field entirely; if a required piece is missing and not in context, use clarify.
