...
/**
 * Carrier Pay Calculation Engine
 * Core logic for calculating carrier pay based on rate profiles and rules
 * Mirrors driverPayCalculation.ts but for carrier partnerships
 * 
 * Key Constraints (V1):
 * - Uses same rate profiles/rules infrastructure as drivers
 * - Profile type must be 'CARRIER'
 ...
 * 2. Carrier's explicit default (isDefault=true)
 * 3. First available profile
 */
function determineProfile(
  assignmentsWithBaseRule: { assignment: Doc<'carrierProfileAssignments'>; minThreshold: number }[],
  ...
  if (applicableProfiles.length === 0) {
    // No profiles match the distance - fall back to carrier's default or first
  ...
  // 2. If tie, prefer carrier's default
...
function evaluateRule(
  ...
  switch (rule.triggerEvent) {
    ...
    case 'TIME_WAITING':
      // For detention - sum up dwellTime from stops if available
...
/**
 * Calculate carrier pay for a leg
 * Called when:
 * 1. Carrier is assigned to leg
 * 2. Stop times are updated
 * 3. Manual "Recalculate" button is clicked
 */
export const calculateCarrierPay = internalMutation({
  args: {
  ...
  handler: async (ctx, args) => {
    ...
    if (!leg.carrierPartnershipId) {
      ...
      return { success: false, reason: 'No carrier assigned' };
    }
    ...
    // 2. Get carrier's profile assignments with BASE rule thresholds
    const assignments = await ctx.db
      .query('carrierProfileAssignments')
      .withIndex('by_carrier_partnership', (q) => q.eq('carrierPartnershipId', carrierPartnershipId))
      .collect();
    ...
    if (!profileId) {
      ...
      await ctx.db.insert('loadCarrierPayables', {
        loadId: leg.loadId,
        legId: args.legId,
        ...
        sourceType: 'SYSTEM',
        isLocked: false,
        warningMessage: 'Carrier has no pay profile assigned. Pay calculated as $0.',
    ...
    if (profile.profileType !== 'CARRIER') {
      return { success: false, reason: 'Profile is not a CARRIER type profile' };
    }
    ...
    // 9. Update load's primaryCarrierPartnershipId cache if needed
...
/**
 * Trigger recalculation for all legs of a load (carrier pay)
...
export const previewCalculation = internalQuery({
  ...
  handler: async (ctx, args) => {
    const leg = await ctx.db.get(args.legId);
    if (!leg || !leg.carrierPartnershipId) {
      return { success: false, reason: 'Leg not found or no carrier' };
    }
    ...
    const assignments = await ctx.db
      .query('carrierProfileAssignments')
      .withIndex('by_carrier_partnership', (q) => q.eq('carrierPartnershipId', leg.carrierPartnershipId!))
      .collect();
    ...
    if (profile?.profileType !== 'CARRIER') {
      return { 
        success: false, 
        reason: 'Profile is not a CARRIER type profile',
        breakdown: []
      };
    }
...
/**
 * Get carrier payables for a load
 */
export const getPayablesForLoad = internalQuery({
  args: {
    loadId: v.id('loadInformation'),
  },
  handler: async (ctx, args) => {
    ...
    // Enrich with carrier partnership info
    const enrichedPayables = await Promise.all(
      payables.map(async (payable) => {
        const partnership = await ctx.db.get(payable.carrierPartnershipId);
        return {
          ...payable,
...
/**
 * Get carrier payables for a specific carrier partnership
 */
export const getPayablesForCarrier = internalQuery({
  args: {
  ...
  handler: async (ctx, args) => {
    ...
    // Calculate totals
    const total = filteredPayables.reduce((sum, p) => sum + p.totalAmount, 0);
    const systemTotal = filteredPayables
      .filter((p) => p.sourceType === 'SYSTEM')
      .reduce((sum, p) => sum + p.totalAmount, 0);
    const manualTotal = filteredPayables
 ...
 */
export const getUnassignedPayables = internalQuery({
  args: {
    carrierPartnershipId: v.id('carrierPartnerships'),
  },
  handler: async (ctx, args) => {
    const payables = await ctx.db
      .query('loadCarrierPayables')
      .withIndex('by_carrier_unassigned', (q) => 
