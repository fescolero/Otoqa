...
|---|---|---|
| Bundled runs + auto-plan | ~~5.1 at scale~~ **SHIPPED 2026-07-26** | Dispatcher-confirmed proposal, deliberately NO cron (auto-committing without review isn't the v8 design; the broker-side `autoAssignment.ts` route-rule engine is a different product surface and stays untouched). `dispatchMobile.suggestPlan`: unassigned AWARDED backlog → greedy time-ordered chaining (deadhead ≤150 mi, drop + 45 min buffer + 45 mph drive-time must make the next pickup window), §5.1 scorer (extracted to shared `rankDriversForOrigin`, reused by `suggestDriversForLoad`) ranks top-3 per run, one driver per plan, unplannable loads reported with reasons. `applyPlan`: per-run commit with assignDriverToLoad's guards, conflicts skipped-and-reported. UI: `/plan` sheet (include/exclude runs, tap driver to cycle candidates, apply summary) + sparkles button on the Board. 7 convex-tests (haversine-real geometry fixture). |
| Voice agent | ~~5.1, 5.2, 5.4~~ **SHIPPED 2026-07-26 (v1, deterministic)** | `expo-speech-recognition ~3.1.3` (sdk-54 dist-tag) with streaming partials; **mic + speech-recognition permissions enter the Dispatch manifest here only** (config plugin). v1 is a deterministic on-device grammar (`apps/dispatch/lib/voice/parser.ts`, 12 unit tests in the new `dispatch` vitest project) — no LLM round-trip: assign / move-window / accept-decline-offer / board+alerts summaries. Every mutating command shows an explicit Confirm card (never acts on speech alone); ambiguous load/driver refs ask instead of guessing. Voice tab added; lazy native require degrades to a "rebuild required" card when the JS is OTA'd onto an older APK. **Device validation still pending** (the New-Arch spike note): confirm streaming partials + on-device availability on the next preview build. **Server upgrade shipped 2026-07-27:** `convex/voice.transcribeAndParse` — Deepgram **Nova-3** (`numerals=true` for numeric trips/mixed HCRs; **Keyterm Prompting** seeded per-request with org driver + active customer names via `voiceContext`) then **Claude Haiku** tool-forced into the same intent shape (`convex/lib/voiceStt.ts`, unit-tested). One mic session feeds both paths: on-device partials live, persisted wav/caf → server; server transcript+intent win, every failure degrades to on-device + deterministic parser; confirm-card flow unchanged. Env vars: `DEEPGRAM_API_KEY` (required), `ANTHROPIC_API_KEY` (optional — absent = grammar parser only). iOS caf-container acceptance by Deepgram = validate when iOS testing starts. **NLU v3 (2026-07-29, device-pending):** date-RANGE driver history ("this week" = Mon→today, "last week" = Mon→Sun prior; inclusive `dateEnd` in both parsers, window widened client-side — `listDriverHistory` unchanged), **clarify intent** (recognizable command missing a required piece → Haiku asks ONE question; the next utterance is combined with the original command server-side via `contextText` and completes it; chained clarifies accumulate), STT-noise hardening (charitable-interpretation prompt rules, possessive "Jorge's loads", "driver X" prefix stripping in matchDriver). Voice tab marker bumped to v3. **NLU v4 (2026-07-29, device-pending): conversational context** — the client sends the last ≤8 visible turns with each utterance (`history` arg); server renders via `buildNluInput` (hard cap 8 turns / 200 chars each — a few hundred Haiku tokens) with prompt rules: reference resolution ONLY ("what about Sam" inherits question+range, "assign it to Marcus" resolves the last-mentioned load), never re-execute confirmed/declined actions, unresolvable referent → clarify/unknown. Confirm-card layer untouched — context shapes proposals, never execution. Feed is in-memory: context resets when the tab unmounts. Marker → v4. |
