...
| Voice agent | ~~5.1, 5.2, 5.4~~ **SHIPPED 2026-07-26 (v1, deterministic)** | `expo-speech-recognition ~3.1.3` (sdk-54 dist-tag) with streaming partials; **mic + speech-recognition permissions enter the Dispatch manifest here only** (config plugin). v1 is a deterministic on-device grammar (`apps/dispatch/lib/voice/parser.ts`, 12 unit tests in the new `dispatch` vitest project) — no LLM round-trip: assign / move-window / accept-decline-offer / board+alerts summaries. Every mutating command shows an explicit Confirm card (never acts on speech alone); ambiguous load/driver refs ask instead of guessing. Voice tab added; lazy native require degrades to a "rebuild required" card when the JS is OTA'd onto an older APK. **Device validation still pending** (the New-Arch spike note): confirm streaming partials + on-device availability on the next preview build. LLM-backed free-form parsing = future enhancement. |
